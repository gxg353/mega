        ...
        for i in range(l):
            ...
            if sql[i] in self.SIGNS:                    
                if i==0 and l>0:
                    ...
                    sql[i+1]=''
                    sql[i-1]=''
            elif str(sql[i]).upper() == 'BETWEEN':
                sql[i+1]='N'
                sql[i+2]='N'
            elif str(sql[i]).upper() == 'LIMIT':
                sql[i+1]='N'
                if i==(l-3):
                    sql[i+2]=',N;'
            else:
...
if __name__ == "__main__":
    ...
    sql="select * from recon_mistakeresults t where t.bankshopperid='8002003' and t.reconresult=3 and t.reconresult!='4' and ISNULL(t.handletype) and ISNULL(t.mark) ORDER BY tobankid limit 70000,2000;"
    sql="SELECT rpo.ID, rpo.MHT_ORDER_NO, rpo.ORDER_NO, rpo.ORDER_TIME, rpo.SETTLE_DATE, iden.CUST_NAME, iden.CERT_NO, rpo.CUST_ACC_NO , rpo.FUND_CODE, rpo.ORDER_STATUS, COALESCE(rpo.ORDER_AMT,0) as ORDERAMT, rpo.MHT_CODE, rpo.FROZEN_STATUS, rpo.BORROW_STATUS, mho.RECON_ORDER_NO, rpo.IS_SPEED_ORDER, rpo.SETTLE_STATUS, rpo.RECON_STATUS, rpo.TRADE_ERROR, COALESCE( rpo.RECON_ERROR,'') as RECON_ERROR, iden.JD_PIN, iden.WALLET_ID from FD_REDEMPTION_ORDER as rpo left JOIN FD_MHT_ORDER as mho on rpo.MHT_ORDER_NO=mho.MHT_ORDER_NO left join FD_FINANCE_IDEN as iden on rpo.CUST_ACC_NO=iden.CUST_ACC_NO where 1=1 and (mho.ORDER_TYPE='Redemption' or rpo.MHT_ORDER_NO=rpo.ORDER_NO) and rpo.ORDER_TIME>= '2014-08-04 00:00:00' and rpo.ORDER_TIME<= '2014-09-09 14:59:59' order by rpo.ORDER_TIME desc limit 160000,10000;"
    sql="select * from recon_mistakeresults t where t.bankshopperid='8002003' and t.reconresult=3 and t.reconresult!='4' and ISNULL(t.handletype) and ISNULL(t.mark) ORDER BY tobankid limit 4000,2000;"
    sql="select a.dt as '日期', case when substr(a.rule_no,1,1)='V' then '人工审核' when substr(a.rule_no,1,1)='E' then '白名单/免疫规则' else substr(a.rule_no,1,1) end as '规则类型', a.rule_no as '规则编码', a.ant as '拦截交易笔数（笔）', format(100*a.ant/b.ant,2) as '交易笔数拦截率（%）', format(a.amt,2) as '拦截交易金额（元）', format(100*a.amt/b.amt,2) as '交易金额拦截率（%）' from ( select dt,rule_no,count(pay_no) as ant,sum(tra_amt) as amt from fct_dat_pay_order_risk_ctl_day Where substr(rule_no,1,1) in('V','E') and dt='2014-08-21' and mer_no in('22792279','22809981','22932972','22933075','22948700','22948717') and biz_type='87' group by dt,rule_no ) a left outer join ( select dt,count(pay_no) as ant,sum(tra_amt) as amt from fct_dat_pay_order_risk_ctl_day where dt='2014-08-21' and mer_no in('22792279','22809981','22932972','22933075','22948700','22948717') and biz_type='87' and (rule_no='' or rule_no='NULL') group by dt ) b on a.dt=b.dt order by a.ant desc limit 0,10;"
    s=SQLParse(sql)
    print s.var_replace()