    ...
    def sync_table_info(self,data=None):
        ...
        for table in data:
            ...
            if db:
                ...
                result,msg=DatabaseManage({"database_name":db_name,"database_ip":self.instance_ip,"database_port":self.instance_port}).add_database()
                if result:
                    db=DatabaseGet().get_database_unique(self.instance_id, db_name)
                    db_id=db.get('id')
                else:
                    db_id=0
            if not db_id:
                log.error('Get database id failed for table :%s' % table)
                continue 
