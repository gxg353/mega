...
def slowlog_statics_per_hour(time):
    '''
    ...
    sql_1="insert into slowlog_sql_day(hash_code,log_time,counts,max_time,avg_time,min_time,max_row,avg_row,min_row)\
         select hash_code,start_time,count(*),max(query_time),avg(query_time),min(query_time),max(rows_examined),avg(rows_examined),min(rows_examined) \
         from slowlog_info where start_time between '%s' and '%s' group by hash_code;" %(_pre_time,_time)
    log.debug(sql_1)
    PyMySQL().execute(sql_1)
    ...
    if data_list and len(data_list) >0 :
        ...
        for data in data_list:
            ...
            if not row_id or row_id == 0:
                _sql="insert into slowlog_time_day(instance_id,db,log_time,counts,lt_one,lt_five,lt_ten,lt_hundred,gt_hundred) values(%s,%s,%s,%s,%s)" % (data.get('instance_id'), 
                                                                                                                                              data.get('dbname'),data.get('start_time'),
                                                                                                                                              data.get('coutns'),(x for x in data.get('count_all')) )
            else:
                _sql="update slowlog_time_day set counts=counts+%s,lt_one=lt_one+%s,lt_five=lt_five+%s,lt_ten=lt_ten+%s,,lt_hundre=lt_huandred+%s,gt_hundred=gt_hundred+%s \
                     where id=%s" %(data,row_id) 
            log.debug(_sql)
